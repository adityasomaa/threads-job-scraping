...
const POOLS: Record<NicheKey, { hero: string; gallery: string[] }> = {
  hospitality: {
...
export function photosFor(niche: NicheKey | undefined): { hero: string; gallery: string[] } {
  const pool = niche && POOLS[niche] ? POOLS[niche] : POOLS.generic;
  return {
    hero: url(pool.hero, 2000),
    gallery: pool.gallery.map((id) => url(id, 1200)),
  };
}
